...
// timings; `PICUS_ANIM_PRESENT_HZ` is an **opt-in diagnostic** anim-only present
// cap (unset = unlimited product path; `0` / `off` / `none` / `false` = no
// throttle; positive Hz caps anim-driven presents only).
